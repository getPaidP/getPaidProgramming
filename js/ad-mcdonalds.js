import * as THREE from 'three';
import {OrbitControls} from 'three/examples/jsm/OrbitControls.js';
import {GLTFLoader} from 'three/addons/loaders/GLTFLoader.js';
import {Box3, BoxHelper} from 'three';
import {PointerLockControls} from 'three/addons/controls/PointerLockControls.js';

// Scene setup
const scene = new THREE.Scene();

const raycaster = new THREE.Raycaster();
const downDirection = new THREE.Vector3(0, -1, 0);

// Camera setup (positioned inside the sphere)
const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 100000);
camera.position.set(174, 76, -115); // Place the camera inside the sphere
camera.lookAt(0, 0, 0);

// Renderer setup
const renderer = new THREE.WebGLRenderer({antialias: true});
renderer.setSize(window.innerWidth, window.innerHeight);
document.body.appendChild(renderer.domElement);


const bgSphere1 = new THREE.Mesh(
  new THREE.SphereGeometry(30, 64, 64),
  new THREE.MeshBasicMaterial({
    map: new THREE.TextureLoader().load("../pics/McDonalds-911.jpg"),
    side: THREE.BackSide
  })
);

bgSphere1.scale.set(9, 9, 9);
bgSphere1.rotation.set(0, 5, 0);
scene.add(bgSphere1);

const bgSphere2 = new THREE.Mesh(
  new THREE.SphereGeometry(30, 64, 64),
  new THREE.MeshBasicMaterial({
    map: new THREE.TextureLoader().load("../pics/mcdonalds90s.jpg"),
    side: THREE.BackSide
  })
);

bgSphere2.scale.set(25, 25, 25);
bgSphere2.rotation.set(0, 5, 0);
scene.add(bgSphere2);

const beefTexture = new THREE.TextureLoader().load("../pics/gross_beef.jpg");
beefTexture.wrapS = THREE.RepeatWrapping;
beefTexture.wrapT = THREE.RepeatWrapping;
beefTexture.repeat.set(100, 100);
const bgSphere3 = new THREE.Mesh(
  new THREE.SphereGeometry(30, 64, 64),
  new THREE.MeshBasicMaterial({
    map: beefTexture,
    side: THREE.BackSide
  })
);

bgSphere3.scale.set(150, 150, 150);
bgSphere3.rotation.set(0, 5, 0);
scene.add(bgSphere3);

const ambientLight = new THREE.AmbientLight("#fff", 3)
scene.add(ambientLight)


let burger;
let mcplate;
let grimaceShake;
let doubleBurg;
let mcSandwich;
let mcSandwichLarge;
let mcdonaldsAtNight;
let mcdonaldsAtNightLowRes;
let mcBuilding;

async function init_models() {
  const loader = new GLTFLoader();

  const loadPromises = [];

  function loadModel(path, onLoadCallback) {
    return new Promise((resolve) => {
      loader.load(path, (glb) => {
        if (onLoadCallback) onLoadCallback(glb);
        resolve(glb);
      }, undefined, (error) => {
        console.error(`Failed to load model: ${error}`);
        resolve(null);
      });
    });
  }

  loadPromises.push(
    loadModel("../3d_models/who_wants_a_creepy_burger.glb", (glb) => {
      burger = glb.scene
      burger.scale.set(25, 25, 25)
      burger.position.set(115, 15, 100)
      burger.rotation.y = 3.5; // 90 deg
      /*
      const box = new Box3().setFromObject(burger);
      const boxHelper = new BoxHelper(burger, 0xffff00);
      scene.add(boxHelper);
      */
      scene.add(burger)
    }, undefined, function (error) {
      console.error(error)
    }),
      
    loadModel("../3d_models/mcdonalds_plate.glb", (glb) => {
      mcplate = glb.scene
      mcplate.scale.set(5, 5, 5)
      mcplate.position.set(-200, 5, 100)

      scene.add(mcplate)

    }, undefined, function (error) {
      console.error(error)
    }),

    loadModel("../3d_models/grimace_birthday_shake_parody.glb", (glb) => {
      grimaceShake = glb.scene
      grimaceShake.scale.set(70, 70, 70)
      grimaceShake.position.set(-20, 18, -80)

      scene.add(grimaceShake)

    }, undefined, function (error) {
      console.error(error)
    }),
    
    loadModel("../3d_models/mcdonalds_double_cheeseburger.glb", (glb) => {
      doubleBurg = glb.scene
      doubleBurg.scale.set(45, 45, 45)
      doubleBurg.position.set(0, 0, 0)

      scene.add(doubleBurg)

    }, undefined, function (error) {
      console.error(error)
    }),

    loadModel("../3d_models/mcdonalds_sandwich.glb", (glb) => {
      mcSandwich = glb.scene
      mcSandwich.scale.set(0.25, 0.25, 0.25)
      mcSandwich.position.set(10, 20, -45)

      scene.add(mcSandwich)

    }, undefined, function (error) {
      console.error(error)
    }),

    loadModel("../3d_models/mcdonalds_sandwich.glb", (glb) => {
      mcSandwichLarge = glb.scene
      mcSandwichLarge.scale.set(150, 150, 150)
      mcSandwichLarge.position.set(0, 0, 0)

      scene.add(mcSandwichLarge)

    }, undefined, function (error) {
      console.error(error)
    }),

    loadModel("../3d_models/mcdonalds_at_night.glb", (glb) => {
      mcdonaldsAtNight = glb.scene
      mcdonaldsAtNight.scale.set(45, 45, 45)
      mcdonaldsAtNight.position.set(0, -100, 0)
      scene.add(mcdonaldsAtNight)
    }, undefined, function (error) {
      console.error(error)
    }),

    loadModel("../3d_models/mcdonalds_at_night_low_res.glb", (glb) => {
      mcdonaldsAtNightLowRes = glb.scene
      mcdonaldsAtNightLowRes.scale.set(45, 45, 45)
      mcdonaldsAtNightLowRes.position.set(0, -100, 0)
      // make invisible
      mcdonaldsAtNightLowRes.visible = false;
      scene.add(mcdonaldsAtNightLowRes)
    }, undefined, function (error) {
      console.error(error)
    }),

    loadModel("../3d_models/mcdonalds_building.glb", (glb) => {
      mcBuilding = glb.scene
      mcBuilding.scale.set(50, 50, 50)
      mcBuilding.position.set(-1800, -500, 0)

      scene.add(mcBuilding)

    }, undefined, function (error) {
      console.error(error)
    }),
  )

  await Promise.all(loadPromises);
  console.log("All models loaded successfully");
}

// Add OrbitControls
/*
const controls = new OrbitControls(camera, renderer.domElement);
controls.update();
controls.enableDamping = true; // an animation loop is required when either damping or auto-rotation are enabled
controls.dampingFactor = 0.25;
controls.enableZoom = true;
*/

//const gridHelper = new THREE.GridHelper(200, 50)
//scene.add(gridHelper)


// controls
const controls = new PointerLockControls(camera, document.body)
controls.connect()
const mouseCenteredCheck = document.querySelector('.mouse-centered')

const speedInput = document.querySelector('.speed-input');
let speed = 15;
speedInput.value = speed;
if (localStorage.getItem('McSpeed')) {
  speed = Number(localStorage.getItem('McSpeed'));
speedInput.value = speed;
}
speedInput.addEventListener('change', function () {
  speed = Number(this.value);
  localStorage.setItem('McSpeed', speed);
speedInput.value = speed;
})

const keys = []
let camera_gravity = 0;
let camHeight = 150;
mouseCenteredCheck.addEventListener('change', function () {
  if (!controls.isLocked) {
    controls.lock()
  }
}, false)
controls.addEventListener('lock', function () {
  mouseCenteredCheck.style.display = 'none'
})
controls.addEventListener('unlock', function () {
  mouseCenteredCheck.checked = false
  mouseCenteredCheck.style.display = 'block'
})
document.addEventListener('keydown', e => {
  if (!keys.includes(e.code)) {
    keys.push(e.code)
  }
})
document.addEventListener('keyup', e => {
  if (keys.includes(e.code)) {
    keys.splice(keys.indexOf(e.code), 1)
  }
})
function updatePos() {
  if (keys.includes("KeyW")) {
    controls.moveForward(speed);
  }
  if (keys.includes("KeyS")) {
    controls.moveForward(-speed);
  }
  if (keys.includes("KeyD")) {
    controls.moveRight(speed);
  }
  if (keys.includes("KeyA")) {
    controls.moveRight(-speed);
  }
  if (keys.includes("Space")) {
    camera.position.y += speed; // Move up
  }
  if (keys.includes("ShiftLeft")) {
    camera.position.y -= speed / 2; // Move down
  }

  // check if in the starting sphere (the 90s one)
  if (isCameraInsideSphere(bgSphere2, camera)) {
    return;
  }

  // check grounded on mcdonalds at night
  raycaster.set(
    new THREE.Vector3(camera.position.x, 1000, camera.position.z),
    downDirection
  );

  const intersects = raycaster.intersectObjects([mcdonaldsAtNightLowRes, mcBuilding]);

  if (intersects.length > 0) {
    const modelHeightAtCam = intersects[0].point.y;
    const cameraHeightAboveModel = camera.position.y - modelHeightAtCam;
    if (cameraHeightAboveModel <= camHeight) {
      camera_gravity = 0; // Reset gravity when grounded
      camera.position.y = modelHeightAtCam + camHeight;
    } else {
      camera_gravity += 1; // Apply gravity when not grounded
      camera.position.y -= camera_gravity;
    }
  }
}

// float function for models inside sphere
function floatModel(model, sphere) {
  // get the sphere's world position
  const sphereWorldPos = new THREE.Vector3();
  sphere.getWorldPosition(sphereWorldPos);

  // prevent the model from going outside the sphere
  const modelWorldPos = new THREE.Vector3();
  model.getWorldPosition(modelWorldPos);
  const distanceToSphereCenter = modelWorldPos.distanceTo(sphereWorldPos);
  const sphereRadius = sphere.geometry.parameters.radius * sphere.scale.x;

}

let lastFrameTime = 0;
const targetFPS = 25;
const frameDuration = 1000 / targetFPS;

// Animation loop
function animate(now) {
  requestAnimationFrame(animate);
  if (now - lastFrameTime < frameDuration) return;
  lastFrameTime = now;
  updatePos()
  renderer.render(scene, camera);
}

async function main() {
  await init_models();
  requestAnimationFrame(animate);
}
main().catch(console.error);

// Handle window resizing
window.addEventListener('resize', () => {
  renderer.setSize(window.innerWidth, window.innerHeight);
  camera.aspect = window.innerWidth / window.innerHeight;
  camera.updateProjectionMatrix();
});


function isCameraInsideSphere(sphereMesh, camera) {
  // Sphere's world position
  const sphereWorldPos = new THREE.Vector3();
  sphereMesh.getWorldPosition(sphereWorldPos);

  // Sphere's world scale (assuming uniform scaling)
  const scale = sphereMesh.scale.x;
  const radius = 30 * scale; // 30 is the geometry radius

  // Distance from camera to sphere center
  const dist = camera.position.distanceTo(sphereWorldPos);

  return dist < radius;
}
